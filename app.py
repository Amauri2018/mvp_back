...
from schemas.pesquisa_web import *
...
pesquisa_tag = Tag(name="Pesquisa", description="Faz o pesquisa de produtos no mercado livre")
...
############################## rotas para API do Mercado Livre #############################################
MELI_BASE_URL = "https://api.mercadolibre.com"

@app.get('/pesquisar_pecas', tags=[pesquisa_tag],
        responses={"200": ListagemPesquisaSchema, "404": ErrorSchema})
def pesquisar_pecas_mercado_livre(query: PesquisaBuscaPeriodoSchema):
    try:
        # requisição do produto pesquisado
        search_url = f"{MELI_BASE_URL}/sites/MLA/search"
        params = {"q": query.pesquisa, "limit": query.limite}
        search_response = requests.get(search_url, params=params)

        # retorna uma JSON, com verios item, referente a pesquisa 
        if search_response.status_code == 200:
            return jsonify(search_response.json())
        else:
            return jsonify({"mensagem": "Erro ao buscar peças no Mercado Livre"}), 500
    except Exception as e:
         return jsonify({}), 500
